...
export function renderSettings(state, handlers) {
  ...
  if (state.modelChoice.mode === "free") {
    ...
    // Providers offered here come straight from data/models.json's "byok"
    // key, generated by scripts/build_data.py — it only lists providers
    // whose API actually allows a direct browser call (see README.md).
    Object.keys(models.byok).forEach((p) => {
      const btn = h(
        "button",
        {
    ...
    modelSection.appendChild(h("p", { class: "muted small" }, "Sent directly from your browser to the provider's own API — never to any server Kimpto controls."));
    modelSection.appendChild(h("p", { class: "muted small" }, "GPT bring-your-own-key isn't available in this static build (OpenAI's API blocks direct browser calls). GPT models are still available in the Free tier above."));
  }
